...
{
    if(NULL == findstart || 10924 > findlen)
    {
        return NULL;
    }
...
void init_mem(machbstart_t *mbsp)
{
    ...
    // check the memory size according to e820map_t structure.
    ...
    mbsp->mb_e820padr = (u64_t)((u32_t)(retemp));       // save the start address of the struct e820map_t
    mbsp->mb_e820nr = (u64_t)retemnr;                   // save the # of elements of struct e820map_t
    mbsp->mb_e820sz = retemnr * (sizeof(e820map_t));    // save the size of the struct array e820map_t
    mbsp->mb_memsz = get_memsize(retemp, retemnr);      // calculate total memory
    init_acpi(mbsp);
    return;
}


// init check CPU function
void init_chkcpu(machbstart_t *mbsp)
{
    ...
    if(!chk_cpu_longmode())
    {
        kerror("Your CPU is not support 64bit mode sys is die!");
        CLI_HALT();
    }
    mbsp->mb_cpumode = 0x40;    // if successed, set teh cpu to 64bit.
    return;
}

// init the stack of the kernel
void init_krlinitstack(machbstart_t *mbsp)
{
    ...
    mbsp->mb_krlinitstack = IKSTACK_PHYADR; // the top of the stack
    mbsp->mb_krlitstacksz = IKSTACK_SIZE;   // the size of the stack is 4KB.
    return;
}
...
void init_bstartpages(machbstart_t *mbsp)
{
    ...
    // physical address start from 0x0
    u64_t adr = 0;
    ...
    // reset top level page index, page pointer to 0.
    ...
    // save the address of the page to machbstart_t struct.
...
// modify the 21th bit from the eflags register to check CPUID
int chk_cpuid()
{
    int rets = 0;
...
// check that whether cpu support long mode
