...
{
    // find the miniOS.bin file from the img file.
    u64_t sz = r_file_to_padr(mbsp, IMGKRNL_PHYADR, "miniOS.bin");
    if (0 == sz)
    {
        kerror("r_file_to_padr err");
    }
    // update the machbstart_t structure
    mbsp->mb_krlimgpadr = IMGKRNL_PHYADR;
    mbsp->mb_krlsz = sz;
    // point to the next start address of the available memory.
